...
class EulerGenerator: 
    ...
    def __init__(self, model, sampling_time=0.05):
        ...
        @param environment<Environment>: The environment to generate the path in
    ...
    def generate(self, initial_paths):
        ...
        for index in range(number_of_corner):
            ...
            lc, a_euler, s_mid = self._calculate_curve_parameters(start, corner, end, theta_end_local)

            start_corner, end_corner = self._calculate_corner_points(start, corner, theta_end_local, lc)
    ...
    def _interpolate_euler_spiral(self, start_corner, path_velocity_corner, time, a_euler, turn_direction, start, end):
        ...
        for j in range(1, number_of_points + 1):
            ...
            if j * self._sampling_time <= time / 2:
                ...
                dphi_local = turn_direction * math.pi * path_velocity_corner * arc_length / math.pow(a_euler, 2)

                local = a_euler * np.array([unit[0], unit[1] * turn_direction])

            else: 
                phi = phi_mid_local + (math.pi / pow(a_euler, 2)) * (-(1.0 / 2) * pow(arc_length, 2) + 2 * arc_length_mid * arc_length - (3.0 / 2) * pow(arc_length_mid, 2))

                dphi_local = turn_direction * math.pi * path_velocity_corner * (2 * arc_length_mid - arc_length) / math.pow(a_euler, 2)

                local = local + velocity_local * self._sampling_time + (1.0 / 2) * acceleration_local * math.pow(self._sampling_time, 2)

            phi_local = turn_direction * phi

            velocity_local = path_velocity_corner * np.array([math.cos(phi_local), math.sin(phi_local)])

            acceleration_local = path_velocity_corner * dphi_local * np.array([-math.sin(phi_local), math.cos(phi_local)])
            
            position.append(np.matmul(rotation_matrix, local.transpose()).transpose() + start_corner)
    ...
    def _calculate_corner_points(self, start, corner, theta_end_local, lc):
        ...
        corner_local = np.matmul(rotation_matrix.transpose(), (corner - start).transpose()).transpose()
        
        start_corner_local = corner_local + lc * np.array([-1, 0])
        
        end_corner_local = corner_local + lc * np.array([math.cos(theta_end_local), math.sin(theta_end_local)])

        start_corner = np.matmul(rotation_matrix, start_corner_local.transpose()).transpose() + start

        end_corner = np.matmul(rotation_matrix, end_corner_local.transpose()).transpose() + start
    ...
    def _calculate_curve_parameters(self, start, corner, end, theta_end_local):
        ...
        lc = epsilon * ((math.sin(beta / 2) / unit_mid[1]) * unit_mid[0] + math.cos(beta / 2))

        lc_check = [np.linalg.norm(end - corner), np.linalg.norm(corner - start)]
